...
void ConfigManipulation::check_chamber_temperature(DynamicPrintConfig* config)
{
    ...
    if (support_chamber_temp_control && config->has("chamber_temperatures")) {
        int limit = config->has("filament_chamber_temp_limit")
                    ? config->option<ConfigOptionInts>("filament_chamber_temp_limit")->get_at(0)
                    : 0;
        if (limit > 0 && limit < config->option<ConfigOptionInts>("chamber_temperatures")->get_at(0)) {
            wxString msg_text = wxString::Format(_L("Current chamber temperature is higher than the material's safe temperature,it may result in material softening and clogging.The maximum safe temperature for the material is %d"), limit);
            MessageDialog dialog(m_msg_dlg_parent, msg_text, "", wxICON_WARNING | wxOK);
            is_msg_dlg_already_exist = true;
            dialog.ShowModal();
            is_msg_dlg_already_exist = false;
